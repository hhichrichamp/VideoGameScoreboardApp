...
public class Scoreboard {
    ...
    public Stats computeStatsForGame(int gameIndex) {
        ...
        for (int i=0; i<this.studentCount; i++){ // go through all students
            Student s = this.students[i]; // get current student
            int[] allHisScores = s.getScores(); // get all his scores
            scoreForGigenGame[i] = allHisScores[gameIndex]; // get score for a particular game at gameIndex
        }
        // now we have  scores for game with gameIdex for all students
        // compute min max and avg
        int min = Utilities.findLowest(scoreForGigenGame);
        int max = Utilities.findHighest(scoreForGigenGame);
        double avg = Utilities.findAveage(scoreForGigenGame);
        Stats statsObj = new Stats(min, max ,avg);
       return statsObj;
